...
仓库里已写好 `deploy.sh`，它会自动：装环境 → 切国内 npm 镜像 → 用 ghproxy 代理拉 GitHub 代码 → 装 pm2 守护 → 启动。
用法：在阿里云控制台实例页点 **「远程连接 / Workbench」** 打开网页终端，把 `deploy.sh` 的内容整段粘贴进去回车即可（IP 已写死为当前服务器 `39.106.141.75`）。
脚本跑完后，**记得去控制台「防火墙」放行 TCP 3000 端口**。
